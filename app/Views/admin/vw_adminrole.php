                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($role as $r) : ?>
                                        <tr>
                                            ...
                                            <td><?= $r['description']; ?></td>
                                            <td>
                                                <a href="<?= base_url('adminrole/roleaccess') . '/' . $r['id']; ?>" class="btn btn-primary btn-xs">Akses</a>
                                                <button href="" class="btn btn-warning btn-xs" onclick="edit('<?= $r['id']; ?>')">edit</button>
                                                <a href="adminrole/delete/<?= $r['id']; ?>" class="btn btn-danger btn-xs" onclick="return confirm('Hapus Data?')">Hapus</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body form">
                <div class="form-group">
                    <form id="frm-modal-role" action="<?= base_url('adminrole/save'); ?>" method="post">
                        <?= csrf_field(); ?>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
...
<script type="text/javascript">
    ...
    function edit(id) {
        ...
        $('.modal-body form').attr('action', "<?= base_url('adminrole/update') ?>");

        $.ajax({
            url: "<?= base_url(''); ?>/adminrole/edit/" + id,
            method: 'post',
            dataType: 'json',
    ...
    function aksesmenu(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('adminrole/edit/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
    ...
    $('.form-check-input').on('click', function() {
        ...
        $.ajax({
            url: "<?= base_url('adminrole/changeaccess'); ?>",
            type: 'post',
            data: {
                menuId: menuId,
                roleId: roleId
            ...
            success: function() {
                document.location.href = "<?= site_url('adminrole/roleaccess/'); ?>" + roleId;
            }
        });
    });
</script>
