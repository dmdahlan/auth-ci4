...
class Adminmenu extends BaseController
{
	...
	public function simpan_menu()
	{
		...
		if (!empty($_POST['status_aktif']))
			$status_aktif = $request->getPost('status_aktif');
		else
			$status_aktif = 0;
		$data = [
			'parent_id'           => $request->getPost('parent_menu'),
			'description'         => $request->getPost('description'),
			'name'                => $request->getPost('name'),
			'sort_menu'           => $request->getPost('sort_menu'),
			'parent_menu'         => $request->getPost('parent'),
	...
	public function update_menu()
	{
		...
		if (!empty($_POST['status_aktif']))
			$status_aktif = $request->getVar('status_aktif');
		else
			$status_aktif = 0;
		$data = [
