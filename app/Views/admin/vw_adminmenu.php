                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" id="parent" name="parent" class="form-control hidd">
...
<script type="text/javascript">
    $(document).ready(function() {
        table = $('#tb_menu').DataTable({
            ...
            ajax: {
                "url": "<?= site_url('adminmenu/ajax_list'); ?>",
                "type": "POST",
            },
        });
        init_select();
    ...
    function simpan_menu() {
        if (method == 'save') {
            url = '<?= site_url('adminmenu/simpan_menu'); ?>';
        } else {
            url = '<?= site_url('adminmenu/update_menu'); ?>';
        }
        $.ajax({
            url: url,
            type: 'POST',
    ...
    function edit_menu(id) {
        ...
        $.ajax({
            url: '<?= site_url('adminmenu/edit_menu/'); ?>' + id,
            type: 'GET',
            dataType: 'JSON',
            success: function(data) {
                $('#id').val(data.id);
    ...
    function hapus_menu(id) {
        swal.fire({
            ...
            if (willDelete.value) {
                $.ajax({
                    url: "<?php echo site_url('adminmenu/delete_menu') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
    ...
    $('#parent_menu').change(function() {
        ...
        if (data != 0) {
            $.ajax({
                url: '<?= site_url('adminmenu/edit_menu/') ?>' + data,
                type: 'GET',
                dataType: 'JSON',
                success: function(data) {
    ...
    function init_select() {
        ...
        const url = '<?php echo base_url('adminmenu/getParentMenu'); ?>';
